...
\chevalier[sec:3.3.5]{Historique des représentations syntaxiques  par des diagrammes en dépendance}{%
   ...
   \begin{quote}
    ...
    \end{quote}
C'est la cinquième colonne qui nous intéresse tout particulièrement, puisqu'elle contient un encodage d'un fragment de structure de dépendance : on y voit que le nom \textit{Paul} est le \textit{nominatif} (c'est-à-dire le \textit{sujet}) de \textit{vient} et que le pronom \textit{le} est l'\textit{accusatif} (c'est-à-dire l'\textit{objet}) de \textit{voir}. Ce type d'encodage a été
redécouvert au 21\textsuperscript{e} siècle et systématisé pour l'encodage des arbres de dépendance (voir l'\encadref{sec:3.5.10} sur \textit{Format tabulaire et treebanks}).
