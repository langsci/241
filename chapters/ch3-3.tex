...
\begin{figure}\label{fig:hierarchie}
\vskip\baselineskip
\caption{Hiérarchisation d'une structure de connexion}
...
\end{tabular}
\todo[inline]{The arrow between the two subfigures indicates a conversion: transformation of the left figure implies the right figure. A big \Longrightarrow would be ok.}
...
\end{figure}
\todo[inline]{Same \Longrightarrow would be ok.}
...
Les notions de valences supérieure et inférieure ont été introduites par Igor Mel’čuk, qui les nomment \textit{valence passive} et \textit{valence active}. Nous préférons garder ces termes pour parler de la partition de la valence selon le point de vue sémantique (voir \chapfuturef{13}).
...
(Voir l’exercice 4 de ce chapitre et sa correction pour une autre formulation du critère due à Paul \citealt{garde1977ordre}.) Si l’on applique ce critère à une unité U = A ${\oplus}$ B dont on veut déterminer la tête, on considèrera le paradigme des éléments A’ qui commutent avec A et le paradigme des éléments B’ qui commute avec B et on comparera les valences supérieures de U~= A ${\oplus}$ B, U’ = A’ ${\oplus}$ B et U” = A ${\oplus}$ B’, c’est-à-dire le paradigme des unités X qui peuvent gouverner ces différentes unités.
...
\loupe{Distorsions sémantique-syntaxe}{%\label{sec:3.3.19}
    ...
    \ea
    \ea  Félix achète un verre à vin (\textit{et le casse}).
    \ex  Félix achète un verre de vin (\textit{et le boit}).\label{ex:3-3-24b}
    \z
    \z
...
\ea
  \ea Un tas de lettres ont été écrites depuis le front pendant la guerre de 14.
  \ex La plupart de ces gens sont d’accord avec moi.
  \z
\z
    ...
    \ea
      \ea[]{\textbf{{Un tas de}}  {lettres et} \textbf{{de}}  {paquets sont arrivés aujourd’hui.}}
      \ex[*]{\textbf{{Plusieurs}}  {lettres et} \textbf{{de}}  {paquets sont arrivés aujourd’hui.}}
      \z
    \z
    ...
    \ea
      \ea {J’ai mangé trop} \textbf{{de} } {chocolat et} \textbf{{de}}  {pain.}
      \ex {J’}\textbf{{en}} \textit{ai mangé trop.}
      \z
    \z
    ...
    \ea
      \ea {Il a fourni} \textbf{{trop}}  {d’efforts} \textbf{{pour abandonner maintenant}}.
      \ex {Il a envoyé} \textbf{{plus}}  {de lettres} \textbf{{que nécessaire}}.
      \z
    \z
    ...
    \ea
      \ea[]{{Je n’ai} \textbf{{jamais}}  {mangé} \textbf{{de chocolat}}.}
...
\ea
\ea[]{Personne ne pense ça.}
\ex[\textsuperscript{??}]{Rien ne pense ça.}
\z
\z
...
\ea
  \ea[]{Elle est venue trois jours / quelques minutes / un samedi matin.}
  \ex[*]{Elle est venue trois tables / quelques amis / une cuillère.}
  \z
\z
...
\ea
  \ea[]{La syntaxe est utile.}
  \ex[*]{Syntaxe est utile.}
  \ex[]{Nous parlons syntaxe.}
  \ex[*]{Nous parlons la syntaxe.}
  \z
\z
...
Dans le même ordre d’idée, il existe des déterminants qui ne sont possibles que dans certains contextes, comme \textit{de} qui s’utilise uniquement dans un contexte négatif ou \textit{ni} qui requiert en plus un deuxième conjoint :

\ea
  \ea[]{Bernd ne mangeait pas de viande.}
  \ex[*]{Bernd mangeait de viande.}
  \ex[]{Bernd ne mangeait ni viande, ni poisson.}
  \ex[*]{Bernd mangeait ni viande.}
  \z
\z
...
\ea
  \ea[]{\textbf{{Sauf abonnement}}  {souscrit pour une durée déterminée […],}  {le client peut interrompre son abonnement à tout moment par lettre recommandée avec demande d’accusé} \textit{de réception.} ({\textup{moncompte.lemonde.fr)}}}
  \ex[]{\textbf{{Sauf sursaut}}  {d’ici à un prochain «~sommet~» européen en avril, les historiens dateront certainement de cette affaire, de ces années 2015--2016, le début de la décomposition de l’Europe.} (lemonde.fr, à propos de la crise des réfugiés syriens)}
  \ex[*]{Sauf l’abonnement …}
  \ex[*]{Sauf un sursaut …}
  \z
\z
...
\ea \textbf{{Chaque ami}}  {a payé sa part.}
...
\ea {Ich habe} \textbf{{den/diesen}  {Apfel}}  {weggeschmissen.}\\
\glt ‘J’ai jeté \textbf{la/cette} \textbf{pomme}.’
\ex {Ich habe} \textbf{{den/diesen}}  {weggeschmissen.}\\
...
\ea {Ich habe} \textbf{{den/diesen}  {faulen} } {weggeschmissen.}\\
\glt ‘J’ai jeté \textbf{la/celle} \textbf{pourrie}.’
\ex {Ich habe} \textbf{{ihn}} (*{faulen})  {weggeschmissen.}\\
...
\ea {Félix a invité} \textbf{{trop d’}}{amis} / \textbf{{des tas d’}}{amis.}\z
...
\loupe{Co-occupation}{%\label{sec:3.3.28}
    ...
    \ea
    \ea   {Je vois} \textbf{{le}}  {livre rouge.}
    \ex   {Je vois} \textbf{{le}}  {rouge.}
    \ex   {Je} \textbf{{le}}  {vois.}
    \z
    \z
...
{L’unité syntaxique formée par les nœuds dominés par un nœud A est appelée la \textstyleTermes{projection maximale} de A.}
...
\maths{Dépendance, dominance et transitivité}{%\label{sec:3.3.31}
    ...
    Formellement, la \textstyleTermesapprof{clôture transitive et réflexive} d’une relation binaire R est la relation R* définie par : \textit{a~}R*\textit{~b} s’il existe $n+1$ nœuds \textit{a}\textsubscript{0}, \textit{a}\textsubscript{1},…, \textit{a\textsubscript{n}} (pour un entier quelconque $n \geq 0$) tels que \textit{a} = \textit{a}\textsubscript{0}, \textit{b} = \textit{a\textsubscript{n}} et \textit{a\textsubscript{i}}~R~\textit{a\textsubscript{i}}\textsubscript{+1} pour tout \textit{i} compris entre 0 et $n-1$. Autrement dit, \textit{a} R* \textit{b} s’il existe un chemin orienté de a à b dans le graphe de la relation R (de longueur éventuellement nulle pour \textit{n} = 0).
...
\ea
  \ea[]{Le chien blanc dort.}
  \ex[*]{Le chien dort blanc.}
  \ex[]{Le chien dort depuis ce matin.}
  \ex[]{Depuis ce matin, le chien dort.}
  \z
\z
...
\exercices{%\label{sec:3.3.35}
    ...
    \exercice{4} Dans son article de \citeyear{garde1977ordre} (p. 8), Paul Garde propose les trois critères suivants pour considérer que B est la tête de l’unité AB :
    ...
    \judgewidth{\textsuperscript{?}*}
     \exi{(1)}[]{Je prends les deux livres.}
     \exi{(2)}[]{Je prends les deux.}
     \exi{(3)}[]{Je les prends tous les deux.}
     \exi{(4)}[]{Je les prends tous.}
     \exi{(5)}[\textsuperscript{?}*]{\textit{Je les prends les deux}. (possible uniquement avec un détachement de \textit{les deux})}
     \exi{(6)}[*]{Je prends tous les deux.}
     \end{exe}
    ...
    \begin{exe}
    \exi{(i)} Le journal Libération a publié un article sur l’affaire Dreyfus.
    \end{exe}
    ...
    \begin{exe}
        \exi{(1)}  J’y suis particulièrement sensible.
        \exi{(2)}  Un tiers des participants seront éliminés dès que le jeu commencera.
        \exi{(3)}  Ma voiture n’a pas voulu démarrer ce matin.
        \exi{(4)}  Le professeur est arrivé deux minutes après la sonnerie.
        \exi{(5)}  Ma chemise a l’air moins blanche qu’avant.
        \exi{(6)}  Epuisée par sa course, Zoé a préféré s’arrêter pour souffler un peu.
        \exi{(7)}  Seuls mes amis peuvent accepter une telle demande.
    \end{exe}
...
\lecturesadditionnelles{%\label{sec:3.3.36}
    ...
    Sur la rection faible, rapidement évoquée en 3.3.19, on pourra consulter \citet{blanche-benveniste1989constructions}. Le fait que certaines prépositions sont des têtes faibles est défendu dans la thèse de \citet{tseng2000selection}.
...
\corrections{%\label{sec:3.3.37}
    ...
    \begin{exe}
    \exi{(i)} Diagramme d’après \citet{buffier1709grammaire}\\
     ...
     [a coutume 
        [de causer
            [beaucoup d’ennui, edge label={%
                            node[near start,left=1em,font=\small\itshape]{régime absolu}}]    
            [à tout le monde, edge label={%
                            node[near start,right=1em,font=\small\itshape]{régime respectif}}]
        ]
     ]
    \end{forest}
    \end{exe}
...
%                                        %
    ...
    \item si A est supprimable dans un contexte, il doit l’être dans tous les contextes :
    ...
    \corrigé{7}
    \begin{enumerate}[label=\alph*.]
    \item Nous avons que dans la \sectref{sec:3.3.13} sur \textit{Le critère distributionnel sans effacement} que la flexion verbale contrôle la distribution de la forme verbale. Pour la même raison, l’auxiliaire, puisqu’il porte la flexion verbale va contrôler la distribution. Voir d’autres arguments dans la \sectref{sec:3.3.8} \textit{Tête d’un énoncé}.
    ...
    \exi{(4)} La difficulté concerne ici le syntagme \textit{deux minutes}. Celui dépend de \textit{après} puisqu’il forme un syntagme avec lui (\textit{deux minutes après}) et que la distribution du tout est celle de \textit{après} (\textit{il est arrivé après} vs *\textit{il est arrivé deux minutes}).
