...
\chevalier[sec:3.3.5]{Historique des représentations syntaxiques  par des diagrammes en dépendance}{%
    Si les analyses en dépendance sont déjà bien développées au 18\textsuperscript{e} siècle (voir la \sectref{sec:3.3.2} sur l'\textit{Historique des notions de dépendance et de tête}), les représentations des analyses syntaxiques par des diagrammes n'apparaissent qu'au 19\textsuperscript{e} siècle. Le premier auteur à utiliser extensivement des diagrammes pour l'enseignement de la grammaire est l'abbé LouiS Gaultier (1746-1818), dont l'\textit{Atlas de grammaire} publié en 1817 contient une grande variété de diagrammes à visée pédagogique et tout particulièrement un diagramme tabulaire encodant  des fragments d'analyse en dépendance. Nous présentons dans la figure \ref{fig:Gaultier1829} un diagramme similaire, extrait des \textit{Élémens de grammaire} publié en \citeyear{gaultier1829elemens} par ses élèves et précédé de la description suivante : 
   \begin{quote}
...
redécouvert au 21\textsuperscript{e} siècle et systématisé pour l'encodage des arbres de dépendance (voir l'\encadref{sec:3.5.10} sur \textit{Format tabulaire et treebanks}).
    ...
    \begin{figure}[H]
    \caption{\label{fig:Gaultier1829}Diagramme d'après \cite{gaultier1817atlas}}
...
Prenons un exemple (sur lequel nous reviendrons à de nombreux reprises).
...
\corrections{%\label{sec:3.3.37}
    \corrigé{1} Nous proposons de traduire l’analyse de Buffier par le diagramme suivant où les flèches représentent les relations du type «~est déterminé par~» ou est «~est particularisé par~». Notons que, à l’exception du cas de \textit{avec des discours}, les deux termes de la relation sont à chaque fois clairement donnés par Buffier. Nous représentons pour cette raison la relation entre \textit{avec des discours} et son gouverneur en pointillé.
