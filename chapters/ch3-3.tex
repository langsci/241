...
{Une \notextstyleTermes{structure de dépendance} est une structure du même type qu’une \hi{structure de connexion}, mais dont certains liens syntaxiques sont des dépendances, c’est-à-dire des \hi{liens hiérarchisés} liant un gouverneur à un dépendant. La structure de dépendance est dite \notextstyleTermes{partielle} quand au moins une connexion n’est pas hiérarchisée.}
...
En français\islang{français}, le principal marqueur illocutoire est la prosodie (transcrit à l’écrit par des signes de ponctuation comme « ?~» ou « !~»). Mais pour les énoncés à tête verbale en français, il existe d’autres marques. Voyons sur un exemple :
...
\globe[sec:3.3.9]{Tête d’un énoncé dans une langue inconnue}{%
  ...
  Commençons par l’\hi{anglais}\islang{anglais}, qui est une langue très proche du français, mais dont le fonctionnement est quand même assez différent. Considérons l’énoncé \textit{Peter has slept} ‘Pierre a (déjà) dormi’. Comme en français, la négation va sur l’auxiliaire sur lequel elle se cliticise :
...
\loupe[sec:3.3.19]{Distorsions sémantique-syntaxe}
    ...
    \noindent On notera également que dans d’autres langues, notamment une langue agglutinante comme le japonais\islang{japonais}, un sens comme ‘il paraît’ est réalisé par un affixe verbal. Ainsi, le verbe {\cjkfont 通じる} \textit{tsūjiru} ‘comprendre’ est suivi d’une série d’affixes modaux terminant sur {\cjkfont そうだ} \textit{sōda} ‘il paraît’.
...
\item le \hi{Critère d’indépendance des codépendants} : un élément n'impose pas à ses codépendants leur forme, leur catégorie ou leur place linéaire (\sectref{sec:3.3.16}).
\end{itemize}
...
\eiffel[sec:3.3.22]{«~Déterminants complexes~»}{%
    Le français\islang{français} possède des syntagmes de la forme «~N \textit{de} N~» ou «~Adv \textit{de} N~», comme \textit{un tas de gens} ou \textit{trop de gens}, où le nom qui suit \textit{de} est clairement la \hi{tête sémantique} (\sectref{sec:3.3.17}) : \textit{un tas de gens} désigne des gens, pas un tas. De plus, dans ces constructions, l’accord se fait généralement avec le nom qui suit \textit{de}, ce qui, en vertu du \hi{Critère morphologique}, désigne également ce nom comme tête :
...
En allemand\islang{allemand}, tout déterminant (autre que \textit{ein} ‘un’ ou \textit{kein} ‘aucun’) peut s’utiliser sans le nom, c’est-à-dire comme pronom (Critère distributionnel avec effacement positif) :
...
\z
    ...
    En anglais\islang{anglais}, la conjonction est étymologiquement liée au démonstratif \textit{that} ‘ça’, mais le phénomène est similaire :
...
Si l’on adopte une représentation syntaxique très granulaire où les syntaxèmes flexionnels occupent des positions distinctes des lexèmes, on distinguera les \notextstyleTermes{projections flexionnelles} des \notextstyleTermes{projections lexicales}. Les projections flexionnelles sont généralement appelées les \textstyleTermes{projections fonctionnelles} dans la littérature en raison de la Syntaxe X-barre qui appelle catégories fonctionnelles les éléments qui ont uniquement une fonction grammaticale (voir l’\encadref{sec:3.4.19} sur la \textit{Syntaxe X-barre}).\largerpage[2]
...
\maths[sec:3.3.31]{Dépendance, dominance et transitivité}{%
    ...
    Donnons une définition plus formelle. Si E est un ensemble, on note E~\textrm{${\times}$}~E l’ensemble des couples sur E. Par exemple, si E = \{ \textit{a,b} \}, alors E~\textrm{${\times}$}~E = \{ (\textit{a,a}), (\textit{a,b}), (\textit{b,a}), (\textit{b,b})~\}. On appelle \textstyleTermesapprof{relation binaire} sur E tout sous-ensemble de E~\textrm{${\times}$}~E. Si R est une relation binaire sur E, on privilégie la notation \textit{a} R \textit{b} pour dire que les éléments \textit{a} et \textit{b} de E sont en relation par R (plutôt que de dire que le couple (\textit{a,b}) est élément de R). Toute relation binaire peut être représentée par un graphe orienté, appelé le \textstyleTermesapprof{graphe de la relation}\is{graphe (d'une relation)}; dont les nœuds sont les éléments de E et une arête de \textit{a} vers \textit{b} représente la relation entre les nœuds \textit{a} et \textit{b} (\textit{a} R \textit{b}). La figure suivante représente le {graphe de la relation} R = \{~(1,2), (2,1), (1,3), (3,3)~\}.
...
La relation de dominance est une relation d’ordre \textstyleTermesapprof{partielle}\is{ordre partiel}~: elle est partielle dans le sens où tous les nœuds ne sont pas ordonnés les uns par rapport aux autres. Par exemple, si deux nœuds sont frères dans l’arbre de dépendance (c’est-à-dire qu’ils ont le même gouverneur), alors aucun des deux ne domine l’autre.
...
\exercices{%\label{sec:3.3.35}
   ...
   \noindent Montrer qu’il s’agit, dans l’intention, des critères distributionnels avec et sans effacement, mais que, pour être corrects, les critères doivent être énoncés et formalisés de manière légèrement différente.
    ...
    \exercice{11} (Construction causative du français.)\islang{français} La construction causative en \textsc{faire} V\textsubscript{inf}
   ...
   \noindent Certains verbes ne permettent qu’un des deux cas :
    ...
    \exercice{13} ({Construction génitive en allemand\islang{allemand} et en anglais.}) En allemand, le complément de nom est au génitif (au lieu d’être marqué par une préposition, comme \textit{de} en français) : \textit{das Haus Olafs} ‘la maison d’Olaf’, litt. la maison Olaf.\textsc{gen}. Mais ce complément génitif peut aussi venir à la place du déterminant comme en anglais\islang{anglais} : all. \textit{Olafs Haus}, angl. \textit{Olaf’s house} ‘la maison d’Olaf’. Quels arguments ces constructions donnent-elles pour ou contre le déterminant comme tête ? Comment les analyseriez-vous ?
}
