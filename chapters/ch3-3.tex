...
\eiffel[sec:3.3.22]{«~Déterminants complexes~»}{%
    Le français possède des syntagmes de la forme «~N \textit{de} N~» ou «~Adv \textit{de} N~», comme \textit{un tas de gens} ou \textit{trop de gens}, où le nom qui suit \textit{de} est clairement la \hi{tête sémantique} (\sectref{sec:3.3.17}) : \textit{un tas de gens} désigne des gens, pas un tas. De plus, dans ces constructions, l’accord se fait généralement avec le nom qui suit \textit{de}, ce qui, en vertu du \hi{Critère morphologique}, désigne également ce nom comme tête :
