    ...
    \begin{tikzpicture}[>={Triangle[]},label distance=-5pt]
        \matrix (matrix) [nodes={CircleNode},
                          ...
                          column sep=-10pt] 
          {
            ...
            \& \& \node [label=below:{\strut‘vin’}, CircleNode] (vin) {}; 
            \& \& \node [label=below:{\strut‘un’}, CircleNode] (un) {};\\
          };
...
\exercices{%\label{sec:3.3.35}
    ...
    \ea
     \ea[]{Je prends les deux livres.}
     \ex[]{Je prends les deux.}
     \ex[]{Je les prends tous les deux.}
     \ex[]{Je les prends tous.}
     \ex[\textsuperscript{?}*]{\textit{Je les prends les deux}. (possible uniquement avec un détachement de \textit{les deux})}
     \ex[*]{Je prends tous les deux.}
     \z
    \z
