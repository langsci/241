...
\Definition{\textstyleTermes{gouverne}, \textstyleTermes{dépend}, \textstyleTermes{gouverneur}, \textstyleTermes{dépendant}}
...
\Definition{\textstyleTermes{tête}}
...
\Definition{\textstyleTermes{structure de dépendance}, \textstyleTermes{partielle}}
...
% %     \includegraphics[width=7cm]{figures/ReedKellogTree}    
    ...
    \includegraphics[width=.33\textwidth]{figures/Kern1883-1.png}
    \includegraphics[width=.33\textwidth]{figures/Kern1883-2.png}\medskip\\
    \raggedright 
...
% %     \includegraphics[width=4cm]{figures/Stemma}
        \begin{tikzpicture}
          ...
          \draw[thick] (ami) -- (votre);
        \end{tikzpicture}
    \caption{Interprétation polygraphique du stemma}
    \end{subfigure}
    \end{figure}
}
...
\ea Pierre veut inviter Marie demain.\z
...
\Definition{\textstyleTermes{tête syntaxique de l’énoncé}}
...
\Definition{\textstyleTermes{subordination}, \textstyleTermes{proposition}, \textstyleTermes{proposition subordonnée}}
...
\ea
  \ea[]{Je crois que, heureusement, Pierre a dormi.}
  \ex[*]{Je crois qu’heureusement que Pierre a dormi.}
  \z
\z
...
\begin{enumerate}
    ...
    \ea
        {Peter} \textbf{{hasn’t}}  {slept.} \\\glt ‘Pierre n’a pas dormi.’
    \z
    Quant à la question, elle est normalement formée par l’antéposition de l’auxiliaire :
    \ea
        \textbf{{Has}}  {Peter slept?} \\\glt   ‘Pierre a-t-il dormi ?’
    \z
    ...
    \ea
      \ea Peter slept.  \\\glt  ‘Pierre a dormi.’
      \ex \textbf{{Did}}  {Peter sleep?}  \\\glt  ‘Pierre a-t-il dormi ?’
      \ex {Peter} \textbf{{didn’t}}  {sleep.} \\\glt ‘Pierre n’a pas dormi.’
      \z
    \z
    ...
    \ea
      \ea {Peter}\textbf{{’s}}  {sleeping} =  {Peter} \textbf{{is}}  {sleeping} \\\glt ‘Pierre est en train de dormir’, lit. Pierre est dormant.
      \ex {Peter}\textbf{{’s}}  {eaten}   =   {Peter} \textbf{{has}}  {eaten}  \\\glt ‘Pierre a (déjà) mangé’
        \z
    \z
...
(Voir l’exercice 4 de ce chapitre et sa correction pour une autre formulation du critère due à Paul \citealt[8]{garde1977ordre}.\todo{Garde 1977 does not have a page no 8}) Si l’on applique ce critère à une unité U = A ${\oplus}$ B dont on veut déterminer la tête, on considèrera le paradigme des éléments A’ qui commutent avec A et le paradigme des éléments B’ qui commute avec B et on comparera les valences supérieures de U~= A ${\oplus}$ B, U’ = A’ ${\oplus}$ B et U” = A ${\oplus}$ B’, c’est-à-dire le paradigme des unités X qui peuvent gouverner ces différentes unités.
...
\Definition{\textstyleTermes{faible}}
...
\Definition{\textstyleTermes{tête interne}, \textstyleTermes{régissant}, \textstyleTermes{régi}}
...
\loupe{Distorsions sémantique-syntaxe}{%\label{sec:3.3.19}
    ...
    \ex  Félix achète un verre de vin (\textit{et le boit}).
    \z
    \z
    ...
    \begin{tikzpicture}[>={Triangle[]},label distance=-5pt]
        \matrix (matrix) [nodes={CircleNode},
                          ...
                          column sep=-10pt] 
          {
            \& \node [label=above:{‘boire’}, CircleNode] (boire) {}; 
...
\loupe{Les grammèmes comme tête}{%\label{sec:3.3.29}
    ...
    \draw [overlay] ($ (defini) !0.5! (ami) $) ellipse [x radius=3cm, y radius=1.5cm,rotate=-45];
    \end{forest}\hfill%
...
\Definition{\textstyleTermes{dominé}}
     ...
     [a coutume 
        [de causer
            [beaucoup d’ennui, edge label={%
                            node[near start,left=1em,font=\normalfont]{régime absolu}}]    
            [à tout le monde, edge label={%
                            node[near start,right=1em,font=\normalfont]{régime respectif}}]
        ]
     ]
    \end{forest}
    \end{exe}
