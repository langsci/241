...
\corrections{%\label{sec:3.3.37}
    ...
    \matrix (matrix) 
        ...
        ] 
      {
        ...
        \node (Matrix-2-1) [draw] {celui}; \& \node (Matrix-2-2) {marche}
                            child { node (Matrix-2-2-1) { ne point } edge from parent[->] } 
                            child { node {dans}
                                    child { node (Matrix-2-2-2) { les ténèbres } 
                                        edge from parent[->] node[right,font=\itshape\footnotesize] {complément} }
                                    edge from parent[->] node[right,font=\itshape\footnotesize] {modification} }; 
    ...
    \node [fit=(Matrix-2-2) (Matrix-2-2-1) (Matrix-2-2-2), draw, inner sep=1pt] (Matrix-Tree) {};
    ...
    \node [fit=(Matrix-1-1) (Matrix-Tree),draw,
           label=60:{\itshape proposition principale}] {}; 
    \draw[->] (Matrix-Tree.north west) to [bend right] (Matrix-2-1.north east);
    \draw[->] (Matrix-2-1) to (Matrix-Lowergroup) node [midway,right=1pt] {\itshape déterminant};
    \end{tikzpicture}
    \z
