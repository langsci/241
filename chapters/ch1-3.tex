...
\maths[sec:1.3.7]{Calcul symbolique et grammaires catégorielles}{%
    ...
    \z
    Cette valeur est calculée en combinant les catégories associées à chaque mot et en simplifiant comme on le fait avec des fractions ordinaires. L’analyse d’une phrase devient un \textstyleTermes{calcul algébrique} similaire au calcul numérique ($a\times{\frac b a}=b$).
...
Pour la traduction automatique, le domaine phare du TAL, on a d’abord essayé de procéder comme le font les humains : analyser le texte source pour en dégager le sens, puis formuler ce sens dans la langue cible. La théorie Sens-Texte présenté dans l’\encadref{sec:1.3.9} qui précède a été initialement développée pour faire de la traduction automatique. Les traducteurs automatiques sont devenus très performants ces dernières années et pourtant ils ne cherchent plus à analyser et comprendre le texte qu’ils traduisent. La traduction repose sur de très grands corpus de textes déjà traduits. On dispose donc de corpus bilingue, avec des textes de la langue source alignés avec des textes de la langue cible. Pour traduire une phrase, on va rechercher les fragments de la phrase qui se trouvent dans le corpus bilingue et les remplacer par un fragment correspondant dans la langue cible. Plus on a de textes déjà traduits, meilleur sera le traducteur automatique. Les traducteurs se sont améliorés dernièrement en utilisant des vecteurs de mots et des techniques d’apprentissage profond basé sur les réseaux de neurones (voir l’\encadref{sec:1.3.11} qui suit sur les vecteurs de mots).

La très grande capacité́ de mémoire de l’ordinateur fait qu’il est souvent plus facile de travailler avec une énorme liste de données (par exemple toutes les constructions possibles pour tous les verbes d’une langue) que de dégager et d’implémenter les régularités derrière (avec leurs exceptions qui existent toujours). Il en résulte que les modèles les plus efficaces pour les applications informatiques ne sont généralement pas les modèles les plus satisfaisants du point de vue de leur « explicativité ». Il n’en reste pas moins que la compréhension du fonctionnement du langage permet de faire progresser le TAL et que, à l’inverse, l’étude des modèles informatiques les plus performants peut donner des intuitions sur le fonctionnement du langage. L’idée générale qui se dégage des modèles informatiques actuels est que les modèles symboliques reposant sur des catégories bien délimitées atteignent rapidement leur limite et que la langue possède de nombreux phénomènes graduels qui ne peuvent être saisis que par des modèles quantitatifs manipulant un très grand nombre de paramètres.
...
Nous allons esquisser la mise en pratique de cette idée. Considérons un mot $m$. On fait donc l’hypothèse que l’on peut réduire le fonctionnement du mot $m$ à sa distribution linéaire, c’est-à-dire à l’ensemble des mots qui se trouvent proche de lui dans la chaîne linéaire du texte. On va donc se fixer une certaine fenêtre d’observation autour de chaque mot, par exemple deux mots avant et deux mots après. On va ensuite regarder toutes les occurrences de $m$ et ramasser pour chaque occurrence de $m$ les mots qui apparaissent dans la fenêtre. L’ensemble de ces mots est supposé décrire la distribution linéaire de $m$ et donc le fonctionnement de $m$. Évidemment, plus le corpus étudié est grand et plus on a d’occurrences de $m$, plus le calcul est fiable. Il faut maintenant traduire ceci en un point dans un espace. L’idée est alors de considérer chaque mot du corpus comme une dimension de l’espace : autrement dit, on va numéroter chaque mot du corpus : \textit{du} sera mot1, textit{est} mot2, textit{chose} mot3, etc. On associe alors au mot $m$ un vecteur dont la première coordonnée est le nombre de fois où mot1 est apparu proche de $m$, la deuxième coordonnée le nombre de fois où mot2 est apparu, etc. Ceci ne marche pas encore car les vecteurs obtenus sont trop grands (l’espace a trop de dimensions) et ils sont trop disparates : des mots au fonctionnement similaire peuvent très bien ne pas être apparus à côté des mêmes mots. Il va donc falloir réduire les dimensions de l’espace et prendre en compte une autre propriété : deux mots ne sont pas seulement similaires s’ils sont proches des mêmes mots, ils sont également similaires s’ils sont proches de mots qui sont eux-mêmes similaires. Il va donc falloir itérer le processus que nous venons de décrire. Nous passons les détails du calcul qui est fort complexe et utilise des méthodes basées sur des réseaux de neurones, qu’on appelle des algorithmes d’\terme{apprentissage profond} (angl. \textit{deep learning}).

Il existe différentes méthodes pour associer des mots à des vecteurs dans un espace multidimensionnel. Ces méthodes sont appelées des \terme{plongements de mots} (angl. \textit{word embeddings}), puisque on « plonge » l’ensemble des mots d’un corpus dans un espace vectoriel. Une fois qu’on a fait ça, on peut tout simplement remplacer les mots par les vecteurs qui leur correspondent. Les systèmes de traitement du texte vont donc travailler avec des vecteurs, c’est-à-dire des tableaux de chiffres, au lieu de travailler avec les mots eux-mêmes, c’est-à-dire avec des symboles, les lettres de l’alphabet. Avant l’introduction des plongements de mots, on enrichissait les systèmes de TAL en injectant des connaissances sur les mots, comme la catégorie syntaxique du mot (nom, verbe, etc.). Ceci devient superflu, car le vecteur de mot contient cette information distributionnelle : si deux mots ont la même catégorie, c’est qu’il partage des propriétés distributionnelles et leurs vecteurs devraient donc avoir certaines similarités.
...
\corrections{%\label{sec:1.3.13}
    \corrigé{1} En modélisant la langue comme une correspondance entre sens et textes, nous laissons de coté plusieurs choses.
