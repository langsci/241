...
Aujourd’hui les modèles capables d’effectuer des correspondances entre sens et textes sont très fragmentaires. On trouve par exemple des modèles de ce type assez performants, mais limités à la production de bulletins météo et manipulant donc un vocabulaire restreint. Les modèles informatiques couvrants se limitent à la correspondance entre textes et représentations syntaxiques de surface. Un tel système est appelé un parseur. 
et ils font aujourd’hui pour cette tâche d’analyse superficielle près d’une erreur par phrase en moyenne.

modèle - tal
distributionalisme

le tal qui est utile au parsing (fin en soi, une des application du tal, ce n'est plus une composante)
ce n'est plus le tal qui améliore le parsing.
réseaux de neurones ont appris qqch qui leur permet de faire le parsing.
on essaie de montrer si on fait une analyse distributionnelle, on met en évidence la structure.
grandes familles = cluster

transformer : la machine a compris qqch sur le lien entre mots
Bert a tellement de connaissances 
un treebank d'un million de mots = 100h de parole = humain en 15 jours.
encodeur décodeur



comme toutes les sciences, la linguistique subit l'impact des modèles d'apprentissage statistique. 
- avant justification pour la science : compréhension = maîtrise des données = avantage économique
- aujourd'hui : grandes masses de données, modèles similaires partout donnent de meilleurs résultats que ce que les humains peuvent gérer
- séparation entre science et ingénierie.
- continu, quantitatif
- pédagogique
- parsing aussi meilleur grâce aux plongements, reproductibilité est une validation d'une analyse syntaxique.
- TA humain meilleur. 
...
Aujourd’hui, beaucoup de modèles en TAL se basent avant tout sur des \textstyleTermes{données} \textstyleTermes{statistiques} apprises sur de grand corpus, car les résultats sont souvent meilleurs et moins coûteux qu’avec des données entièrement développées à la main qui s’avèrent généralement incomplètes. La très grande capacité de mémoire de l’ordinateur fait qu’il est souvent plus facile de travailler avec une énorme liste de données (par exemple toutes les constructions possibles pour tous les verbes d’une langue) que de dégager et d’implémenter les régularités derrière (avec leurs exceptions qui existent toujours). Il est possible que les modèles les plus efficaces pour les applications informatiques ne soient pas les modèles les plus satisfaisants du point de vue de leur «~explicativité~». À l’heure actuelle, les traducteurs automatiques les plus performants sont ceux qui interrogent de grands corpus bilingues pour rechercher tous les fragments du texte pour lesquels on a déjà une traduction et qui choisissent la plus fréquente (\hi{modèle de traduction}). Ainsi les traductions sont combinées et lissées à l’aide de corpus unilingues encore plus grands (\hi{modèle de langue}). De tels systèmes de traduction statistiques obtiennent de meilleurs résultats que les systèmes qui cherchent à simuler un traducteur humain, c’est-à-dire qui cherchent à calculer une représentation sémantique du texte à traduire pour générer à partir de celle-ci un texte dans une autre langue.

- ajouter distribution dans les modèles récents

- pédagogie + besoins explicatifs -> linguistique formelle reste intéressante
...
\lecturesadditionnelles{%\label{sec:1.3.12}
    ...
    Le raisonnement par abduction a été dégagé par le philosophe américain, Charles S. Peirce (1839--1914). En \citeyear{peirce1903harvard}, il en donne la formulation suivante : «~The surprising fact, C, is observe ; But if A were true, C would be a matter of course ; Hence, there is reason to suspect that A is true.~».
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \corrigé{5} La notion de sujet syntaxique sera définie dans le \chapfuturef{18}. L’accord du verbe est une des propriétés définitoires du sujet en français, avec la position privilégiée avant le verbe ou l’emploi de pronoms comme \textit{il} ou \textit{on}.
