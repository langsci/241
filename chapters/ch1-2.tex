...
\lecturesadditionnelles{%\label{sec:1.2.17}
    Si l’on est intéressé par l’inventaire des significations morphologiques que les langues nous obligent à communiquer, on pourra consulter le deuxième volume du monumental cours de morphologie d’Igor Mel’čuk. La citation de Jakobson de  l’encadré \encadref{fig:1.2.12}  est extraite d’un article très lisible. On peut aussi lire le très bon article publié dans le New York Times Magazine par Guy Deutscher. L’article d’Edward Sapir sur le nootka se trouve dans sa sélection d’écrits. Les autres sujets abordés dans ce chapitre seront largement développés dans la suite de l’ouvrage.
...
\corrections{%\label{sec:1.2.19}
    ...
    \exi{(i)}
    \begin{xlista}
     \ex Ali pleure parce que Zoé est partie.
     \ex Le départ de Zoé a fait pleurer Ali.
    \end{xlista}
    \exi{(ii)}
    \begin{xlista}
      \ex Les Chinois consomment de plus en plus de viande.
      \ex La consommation de viande des Chinois a augmenté.
    \end{xlista}
    \end{exe}
