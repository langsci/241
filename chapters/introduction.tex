...
Il en découle que cet ouvrage n’est pas vraiment un ouvrage d’introduction : même s’il tente d’élaborer son objet à partir de rien et qu’il est donc en théorie accessible sans pré-requis, cet ouvrage fait certainement appel à une \hi{maîtrise du raisonnement scientifique} qui ne s’acquière qu’avec l’expérience. On pourra comparer, toutes choses égales par ailleurs, une tentative de ce genre à celle du groupe de mathématiciens français rassemblés sous le pseudonyme de Nicolas Bourbaki qui rédigea un ouvrage de construction des mathématiques à partir de rien (le premier tome démarre par la construction des entiers à partir du seul ensemble vide). L’œuvre de Nicolas Bourbaki, si elle est élémentaire au sens premier du terme (comme le souligne son titre, \textit{Éléments} \textit{de mathématiques}), s’avère d’une lecture bien difficile pour des non-mathématiciens.
...
\loupe{La syntaxe et les autres domaines de la linguistique}{
    ...
    Celles-ci et beaucoup d’autres questions que nous étudions dans ce livre concer\-nent la \textstyleTermes{syntaxe}.
    ...
    La \textstyleTermes{phonologie} s’interroge sur ces questions. Nous les aborderons dans la mesure où la structure phonologique des énoncés nous offre de nombreux indices sur la structure syntaxique des énoncés et où la langue parlée constitue, du notre point de vue, un bien meilleur sujet d’étude que la langue écrite.
    ...
    S’il s’agit de répondre à ces questions en termes de fonctionnement de la langue et de l’humain produisant les sons, on se trouve dans la \textstyleTermes{phonétique (articulatoire)}, si c’est en termes purement techniques et physiques, c’est plutôt l’\textstyleTermes{acoustique} et le \textstyleTermes{traitement du signa}l, un domaine relevant de la physique, qui sont concernés.
...
\noindent\begin{minipage}[c]{24pt}
  {\includegraphics[width=24pt]{figures/tbls-explore.pdf}}\end{minipage}\hfill\begin{minipage}[c]{\textwidth - 36pt}des encadrés sur le \hi{français}, qui reste la langue privilégiée pour illustrer notre propos.\end{minipage}
...
\exercices{
\todo[inline]{here there is no vspace between the exercises. Sometimes there is some space and in some chapters the space is quite big. I don't know how it is calculated, by I suspect that the vfills are too big.}
