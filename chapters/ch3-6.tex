...
\begin{tikzpicture}[>={Triangle[]},
	...
	\begin{scope}[nodes={CircleNode}]
		...
		\node (singulier1) [below left=of prop,label=below:{singulier}] {};
		\node (def) [below right=of prop,label=below:{défini}] {};
	\end{scope}
	
	\draw[-{Triangle[]}] (tenirlajambe) -- (zoe) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (durer) -- (tenirlajambe) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (durer) -- (heure) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (tenirlajambe) -- (prop) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (indef) -- (heure) node[font=\footnotesize,midway,right] {1};
	\draw[-{Triangle[]}] (singulier1) -- (prop) {};
	\draw[-{Triangle[]}] (def) -- (prof) {};
	\draw[-{Triangle[]}] (singulier0) -- (heure) {};
...
Les arguments d’un sémantème sont numérotés dans l’\terme{ordre d’oblicité} croissante. L’ordre d’oblicité et son inverse, l’\terme{ordre de saillance}, seront définis dans le \chapfuturef{18}. Disons juste que le sujet est la relation la plus saillante, suivie du complément d’objet indirect, puis du complément d’objet indirect. Le sujet, lorsqu'il est un argument du verbe, est donc le premier argument et par conséquent le \hi{premier actant}. Pour les modifieurs, le gouverneur syntaxique est considéré comme le premier argument. Ceci est justifié par le fait que lorsqu’un modifieur est verbalisé, son gouverneur devient le sujet de la construction (\textit{une \underline{maison} \textbf{blanche}}, \textit{la \underline{maison} \textbf{est blanche}}). Le deuxième argument d’un modifieur est appelé le deuxième actant (même s’il n’est pas à proprement parler le deuxième des actants).
...
\noindent (Il existe une autre acception de \textit{avoir peur} qui est une collocation, où \textsc{peur} est modifiable et \textit{avoir} commute avec \textsc{faire} : \textit{j’ai une peur bleue des araignées}. Mais le sens figuré utilisé en \REF{ex:accompli} est bien un phrasème \phraseme{avoir peur}.)
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (causer) -- (zoe) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (causer) -- (peur) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (intense) -- (peur) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (peur) -- (moi) node[font=\footnotesize,midway,right] {1};
...
\end{forest}
\caption{Structures sémantique et syntaxique profonde de \REF{ex:peurbleue}\label{fig:peurbleue}}
\end{figure}
...
\end{forest}
\caption{Structure syntaxique profonde de \REF{ex:content}a \label{fig:content}}
\end{figure}
...
Nous avons déjà parlé du clivage dans la section \sectref{sec:11.8} sur \textit{Les tests de constituance} et dont nous reparlerons dans le \chapfuturef{20}. Le clivage, réalisé par \textit{c’est} X \textit{qui/que} Y, s’applique à une proposition Y dont il promeut l’un des éléments X. Il possède donc deux actants : l’élément promu X est le premier actant et la proposition Y privée de cet élément est le deuxième actant.
...
\end{forest}
\caption{Structure syntaxique profonde de \REF{ex:13-clivage}a et b \label{fig:13-clivage}}
\end{figure}
...
\end{forest}
\caption{Structure syntaxique profonde de \REF{ex:13-passif}a et b \label{fig:13-passif}}
\end{figure}
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (penser) -- (zoe) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (penser) -- (venir) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (venir) -- (zoe) node[font=\footnotesize,midway,above] {1};
	
\end{tikzpicture}
\caption{Structure sémantique commune à \REF{ex:13-pronom}a et b \label{fig:13-pronom-sem}}
\end{figure}
...
\begin{forest} for tree={font=\normalfont}
	...
	\draw[->,dashed] (venir) to[out=south,in=south] node[pos=0.5,below]{\footnotesize 1} (zoe);
\end{forest}
\caption{Structures syntaxiques profondes de \REF{ex:13-pronom}a et b \label{fig:13-pronom-pro}}
\end{figure}
...
\end{forest}
\caption{Structures sémantique et syntaxique profonde pour \REF{ex:13-fumer}\label{fig:13-fumer} }
\end{figure}
...
\end{forest}
\caption{Structures syntaxiques profondes de \REF{ex:13-suivant}a et b\label{fig:13-suivant} }
\end{figure}
...
\end{forest}
\caption{Structure syntaxique profonde de \REF{ex:13-approx}b\label{fig:13-approx}}
\end{figure}

La \hi{dislocation} est un cas de sémantème constructionnel, comme le clivage, qui, à la différence du clivage, n’a pas vraiment de marqueur lexical : la dislocation met en jeu un pronom qui reprend l’élément disloqué, mais le pronom n’est pas en soi le signifiant d’une dislocation. Nous considérons donc qu’il s’agit d’un sémantème caché, que nous notons « disloc » (voir la figure \ref{fig:13-disloc}).
...
\end{forest}
\caption{Structure syntaxique profonde de \REF{ex:13-disloc}\label{fig:13-disloc}}
\end{figure}
...
\end{forest}
\caption{Structure syntaxiques profondes de \REF{ex:13-vin}a et b \label{fig:13-vin}}
\end{figure}
...
\end{forest}
\caption{Structure syntaxiques profondes de \REF{ex:13-excellent}a et b \label{fig:13-excellent}}
\end{figure}
...
Parmi les compléments locatifs, on distingue les modifieurs qui indiquent le lieu et le moment du procès d’arguments qui indiquent une destination. Par exemple, \textsc{aller} est bivalent (quelqu’un va quelque part), \textsc{mettre} est trivalent (quelqu’un met quelque chose quelque part). Certains cas sont délicats à trancher : par exemple, le complément \textit{à la bibliothèque} est un modifieur s’il indique le lieu où je travaille aujourd’hui, mais il est un argument si \REF{ex:travailler} est utilisé pour dire ‘je suis un employé de la bibliothèque’.
...
Dans le représentation syntaxique profonde de \REF{ex:13-soeur}a, nous indiquons explicitement le fait que \textsc{sœur} est translaté en V (voir la figure \ref{fig:13-soeur-synt}a). C'est seulement dans ce cas que le sémantème \textsc{sœur} peut réaliser son argument.
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (soeur) -- (zoe) node[font=\footnotesize,midway,left] {1};
...
\caption{Structures sémantiques de \REF{ex:13-soeur}a et b \label{fig:13-soeur-sem}}
\end{figure}
...
\end{forest}
\caption{Structures syntaxiques profondes de \REF{ex:13-soeur}a et b \label{fig:13-soeur-synt}}
\end{figure}
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (etre) -- (mars) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (etre) -- (planete) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (troisieme) -- (planete) node[font=\footnotesize,midway,right] {1};
	\draw[-{Triangle[]}] (appartenir) -- (planete) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (appartenir) -- (systeme) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (solaire) -- (systeme) node[font=\footnotesize,midway,left] {1};
...
\end{forest}
\caption{Structure syntaxique profonde de \REF{ex:13-Mars}\label{fig:13-Mars}}
\end{figure}
...
\end{forest}
\caption{Structures syntaxiques profondes de \REF{ex:chez}a, b et c \label{fig:chez}}
\end{figure}
...
\end{forest}
\caption{Structures syntaxiques profondes de \REF{ex:13-francais}a et b \label{fig:13-francais}}
\end{figure}
...
\caption{Tableaux de régime de \phraseme{tenir la jambe}, \textsc{question}, \textsc{chez}, \textsc{heureux}\label{fig:13-regime}}
\end{figure}
...
\adjustbox{valign=t}{\begin{tikzpicture}[framed,
		...
		\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
			\node (nil) [below right=of prof, label=right:{[définitude]}] {};
		\end{scope}
...
\adjustbox{valign=t}{\begin{tikzpicture}[framed,
		...
		\begin{scope}[nodes={CircleNode}]
			\node (A) [label=above:{[1:N]}] {};
			\node (le) [fill,below=of A,label=below:{\textsc{le}}]{};
		\end{scope}
		
		\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
			\node (def) [fill,below right=of A,label=right:{déf}] {};
		\end{scope}
...
\begin{tikzpicture}[framed,
	...
	\begin{scope}[nodes={CircleNode}]
		\node (A) [below of=nil,label=above right:{[1:V]}] {};
	\end{scope}
	
	\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
		\node (ind) [below right=of A,label=right:{ind}] {};
	\end{scope}
...
\begin{tikzpicture}[framed,
	...
	\begin{scope}[nodes={CircleNode}]
		...
		\node (B) [below right=of avoir,label=above right:{[1:V]}] {};
	\end{scope}
	
	\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
		\node (indpres) [fill,right=of avoir,label=right:{ind.prés}] {};
		\node (partpasse) [fill,below right=of B,label=right:{part-passé}] {};
	\end{scope}
...
\node (B) [below=of a,label=below:{[2:N]}] {};
\node (le) [fill,below=of jambe,label=below:{\textsc{le}}] {};
\end{scope}

\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
\node (def) [fill,below right=of jambe,label=right:{déf}] {};
\node (nil) [ right=of tenir, label=right:{[mode]]}] {};
\end{scope}
...
\gkcaption{$\ulcorner$\textsc{tenir la jambe}$\urcorner$}
...
\begin{tikzpicture}[framed,
	...
	\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
		\node (nil) [below right=of prof, label=right:{[déf]]}] {};
	\end{scope}
...
\begin{tikzpicture}[framed,
	...
	\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
		\node (nil) [below right=of heure, label=right:{[déf]]}] {};
	\end{scope}
...
\begin{tikzpicture}[framed,
	...
	\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
		\node (indef) [fill,below right=of A,label=right:{indéf}] {};
	\end{scope}
...
\begin{tikzpicture}[framed,
	...
	\begin{scope}[nodes={CircleNode}]
		\node (A) [label=above:{[1:V]}] {};
		\node (pendant) [fill,below=of A,label=right:{\textsc{pendant}}] {};
		\node (B) [below=of pendant,label=below:{[2:N]}] {};
	\end{scope}
...
\begin{tikzpicture}[label distance=0pt,
	...
	\begin{scope}[nodes={DiamondNode},node distance=0.1cm and 0.1cm]
		\node (indpres) [fill,right=of avoir,label=right:{ind.prés}] {};
		\node (partpasse) [fill,right=of tenir,label=right:{part-passé}] {};
		\node (def0) [fill,below right=of jambe,label=right:{déf}] {};
		\node (def1) [fill,below right=of prof,label=right:{déf}] {};
		\node (indef) [fill,below right=of heure,label=right:{indéf}] {};
	\end{scope}
...
\end{forest}
\caption{Structure référentielle avec ancrage	pour \REF{ex:13-portée}a et b\label{fig:13-portée}}
\end{figure}
...
\end{forest}
\caption{Représentations syntaxiques profondes avec un verbe à contrôle pour \REF{ex:13-venir}a et b \label{fig:13-venir}}
\end{figure}
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (penser) -- (zoe) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (penser) -- (venir) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (venir) -- (zoe) node[font=\footnotesize,pos=0.25,above] {1};
...
\end{forest}
\caption{Représentations syntaxiques profondes des constructions à verbe support \REF{ex:13-gifle}a et b \label{fig:13-gifle}}
\end{figure}
...
\begin{forest} for tree={font=\normalfont}
	...
	[\textsc{Zoé},name=zoe,edge label={node[midway,above left=-0.5ex,font=\footnotesize]{$+$}}]
...
\begin{forest} for tree={font=\normalfont}
	...
	[\textsc{Zoé},edge label={node[midway,above left=-0.5ex,font=\footnotesize]{1}}]
	[\textsc{Ali},name=ali,edge label={node[midway,left,font=\footnotesize]{$+$}}]
	[\textsc{sympa},name=sympa,edge label={node[midway,above right=-0.5ex,font=\footnotesize]{2}}]
	]
	\draw[->,dashed] (sympa) to[out=south,in=south] node[pos=0.5,below]{\footnotesize 1} (ali);
\end{forest}
\caption{Représentations syntaxiques profondes avec un verbe à montée pour \REF{ex:13-dormir}b et \REF{ex:13-trouver}a \label{fig:13-monte}}
\end{figure}
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (causer) -- (zoe) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (causer) -- (rire) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (rire) -- (ali) node[font=\footnotesize,midway,right] {1};
...
\end{forest}
\caption{Représentations sémantique et syntaxique profonde de la construction causative \REF{ex:13-aux}b \label{fig:13-causatif}}
\end{figure}
...
%b. Félix <-1- boire\_prés ..2..> vin -+-> verre\_sg, indéf -1-> (vin)
\begin{forest} for tree={font=\normalfont}
...
\end{forest}
\caption{Structures syntaxiques profondes de déterminants insérés pour \REF{ex:13-insertion}a et b \label{fig:13-insertion}}
\end{figure}
...
\item	les relations syntaxiques pures, qui ne se superposent pas à une relation sémantique ; elles sont étiquetées $+$ ou (1), (2), etc. ;
...
D’autres exemples de structures syntaxiques profondes seront donnés dans le \chapfuturef{19} sur l’\textit{Extraction} et le \chapfuturef{20} sur les \textit{Listes paradigmatiques}.
...
\exercice{5}~(noms temporels)\ \ Quel problème posent les noms temporels pour la modélisation ? Comment modéliser les énoncés suivants en syntaxe profonde ?
\begin{enumerate}[label=\alph*.]
...
Le flambeau a été repris par son étudiant, Alain Polguère, dont nous recommandons encore une fois l’ouvrage de sémantique lexicale et lexicologie. Celui-ci développe également un lexique sémantique et syntaxique électronique sous forme de réseau lexical, consultable en ligne sur https://lexical-systems.atilf.fr/spiderlex/. Dans les travaux de Mel’čuk et Polguère, l’accent est particulièrement mis sur la combinatoire lexicale restreinte décrite à l’aide des fonctions lexicales, dont nous n’avons donné qu’un très faible aperçu dans ce chapitre.
...
\corrections{%
\corrigé{1} La plupart de ces complément de nom sont des actants : \textsc{réponse} est un nom prédicatif dont \textsc{Zoé} est le premier argument ; \textsc{portrait} est un nom prédicatif à deux arguments (quelqu’un fait le portrait de quelqu’un) et \textsc{Zoé} peut être l’un ou l’autre des arguments ; \textsc{main} désigne une partie du corps de quelqu’un ;  \textsc{trousse} désigne un artefact, c’est-à-dire un objet fabriqué pour être utilisé et dont l’utilisateur est donc un argument ; \textsc{chat} désigne un animal domestique, qui à ce titre possède un maître/utilisateur (un animal, lorsqu’il est domestiqué, peut être vu comme une sorte d’artefact). Il est également défendable de considérer \textit{de Zoé} comme un modifier dans ces deux derniers exemples. On traitera alors la préposition \textsc{de} comme la réalisation d’un sémantème «~possesseur~». Dans les deux derniers exemples, les compléments locatifs \textit{de Bretagne} et \textit{du Cap Fréhel} peuvent être clairement considérés comme des modifieurs. Nous considérons que la préposition \textsc{de} est la réalisation d’un sémantème « provenance » dans le premier cas et « location » dans le deuxième.
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (beaucoup) -- (gens) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (aimer) -- (gens) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (conditionnel) -- (aimer) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (aimer) -- (passer) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (passer) -- (gens) node[font=\footnotesize,midway,above] {1};
	\draw[-{Triangle[]}] (passer) -- (noel) node[font=\footnotesize,midway,right] {2};
	\draw[-{Triangle[]}] (passer) -- (laponie) node[font=\footnotesize,midway,below] {3};
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\begin{scope}[nodes={CircleNode}]
		...
		\node (zoe) [below=of declarer,label=below:{`Zoé'}] {};
...
\begin{forest} for tree={font=\normalfont}
	...
	[$\ulcorner$\textsc{y aller}$\urcorner$,edge label={node[midway,above right=-0.5ex,font=\footnotesize]{1}}
	[\textsc{on},edge label={node[midway,right,font=\footnotesize]{1}}]
	]
	]
\end{forest}
\caption{Structures sémantique et syntaxique profonde de a \label{fig:13-vocatif} }
\end{figure}
...
\begin{forest} for tree={font=\normalfont}
	...
	[tough-mvt(\textsc{difficile}),name=mvt,edge label={node[midway,right,font=\footnotesize]{$+$}}
...
\begin{forest} for tree={font=\normalfont}
	...
	[\textsc{livre}\textsubscript{sg},name=livre,edge label={node[midway,above left=-0.5ex,font=\footnotesize]{$+$}}
	[\textsc{cet},edge label={node[midway,left,font=\footnotesize\itshape]{\textsc{mod}}}]
	]
...
\begin{tikzpicture}[>={Triangle[]},
	...
	\draw[-{Triangle[]}] (lire) -- (generique) node[font=\footnotesize,midway,left] {1};
	\draw[-{Triangle[]}] (lire) -- (livre) node[font=\footnotesize,midway,right] {2};
