...
\begin{figure}
	\begin{subfigure}[h]{0.5\textwidth}
		...
		\end{tikzpicture}
		\caption{Structure prédicative sémantique}\todo[inline]{put figure a on the left (its caption is to close from figure c)}
	\end{subfigure}%
	\bigskip\\%
...
La diathèse active est la diathèse de base en français pour les verbes, puisqu'elle est plus courante et nettement moins marquée que la diathèse passive, qui recourt, elle, à un auxiliaire et une préposition. C'est donc l'agent qui sera le premier argument d'un verbe transitif en français et le patient le deuxième argument. (Voir le \chapfuturef{18} pour une discussion plus approfondie sur les notions de sujet, d'agent, et de patient et le cas des langues ergatives, où la diathèse passive peut être la diathèse de base.)

\Definition{\terme{redistribution}, \terme{voix}}
{On appelle \terme{redistribution} toute construction qui suppose un changement de diathèse par rapport à la diathèse de base. On appelle \terme{voix} les grammies qui ont pour uniquement signification le marquage de la diathèse.}
...
Nous avons déjà parlé du clivage dans la \sectref{sec:3.4.10} sur \textit{Les tests de constituance} et dont nous reparlerons dans le \chapfuturef{20}. Le clivage, réalisé par \textit{c’est} X \textit{qui/que} Y, s’applique à une proposition Y dont il promeut l’un des éléments X. Il possède donc deux actants : l’élément promu X est le premier actant et la proposition Y privée de cet élément est le deuxième actant.
...
\end{tikzpicture}}
\todo[inline]{If possible, align the nodes of the three diagrams and align the top of the boxes as weel as the bottom of two boxes on right}
...
\todo[inline]{Is it possible to maintain the same length for the dependencies in this tree than in other trees? Moreover the leaves of the tree are not aligned}
...
\todo[inline]{Same problem as previously: nodes of the same level are not aligned and the vertical distance between levels is bigger than in other trees}	
\end{tikzpicture}
...
\end{forest}
\todo[inline]{If possible, add a small space between the arrow from pendant to “tenir la jambe” and the two other arrows}
...
Les constructions à contrôle se produisent quand la structure prédicative contient un cycle (non orienté). Ce cycle doit être réalisé en syntaxe de surface par un arbre, c’est-à-dire par une structure acyclique. Il faut donc couper le cycle quelque part.
...
\todo[inline]{Subfigure a is missing. This is the subfigure b.}
\end{figure}
...
\begin{figure}\label{fig:13-insertion}
%FIGURE 31
...
\begin{itemize}
\item	les relations de coréférence, qui indique la scission d’un nœud sémantique ; elles sont représentées par une flèche bidirectionnelle en pointillés ;
\item	les relations d’ancrage, qui indique l’ancrage du référent d’un indéfini sur un autre élément de la structure ou sur l’univers du discours ; elles sont représentées par une flèche en pointillés.
\end{itemize}
...
%FIGURE 32
	...
	\begin{subfigure}[h]{\textwidth}
		...
		\end{tikzpicture}
		\caption{Structure prédicative sémantique}\todo[inline]{the caption of subfigure a is closer to subfig b than subfig a. This a general problem of subfigures  below each other, increased here by the asymetry of subfig a and the space above the caption}
	\end{subfigure}%
	\bigskip\\%
