...
\end{figure}
\todo[inline]{\textbackslash Longdownarrow}
...
\begin{figure}[H]
    ...
    \textit{The man hit the ball. 'L'homme frappa le ballon.’\label{fig:chomsky57}}
\end{figure}
...
\end{figure}
\todo[inline]{\textbackslash Rightleftarrow}
...
\end{figure}
\todo[inline]{Add a \textbackslash Rightleftarrow between the two diagrams}
    ...
    Dans un arbre binaire, il en va tout autrement, puisque, comme nous l’avons montré ci-dessus (\sectref{sec:3.4.21} sur \textit{Arbres de constituants binaire et polygraphe}), \textbf{chaque branchement correspond à une unique connexion}. Si un tel arbre contient un branchement ternaire, il doit être interprété comme une \textbf{connexion ternaire} (voir l’\encadref{fig:3.2.17} éponyme). La syntaxe X-barre, qui revendique l’usage des arbres binaires, s’est aventurée à proposer des branchements ternaires pour la coordination (notamment dans l'ouvrage de \citealt{jackendoff1977x}). On trouve déjà les mêmes arbres dans \citealt{hockett1958course} ; la figure \ref{fig:hockett-coord} les boîtes de Hockett pour la coordination où le branchement est ternaire et symétrique, avec une position bien différenciée pour la conjonction).
...
%     \includegraphics{}
    ...
    \todo[inline]{I draw it. Is it possible to draw it again?}
\end{figure}
...
\subsubsection{Constructions exocentriques}
   ...
   \subsubsection{Unités syntaxiques}
   ...
   \subsubsection{Interface sémantique-syntaxe}
