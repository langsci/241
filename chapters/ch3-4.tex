...
\begin{figure}
\begin{tikzpicture}[
                     every node/.style={font=\itshape, inner sep=6pt}, 
...
\begin{figure}
\fbox{\begin{tikzpicture}[every node/.style={font=\strut}]
\begin{scope}[
    every node/.style={circle,fill=black},
...
\end{scope}
\node[below=1pt of root-1-1] (beaucoup) {beaucoup};
\node[below=1pt of root-1-2-1] (de) {de};
\node[below=1pt of root-1-2-2] (gens) {gens};

\node[below=1pt of root-2] (aimeraient) {aimeraient};

\node[below=1pt of root-3-1]   (passer) {passer};
\node[below=1pt of root-3-2]   (Noel) {Noël};
\node[below=1pt of root-3-3-1] (en) {en};
\node[below=1pt of root-3-3-2] (Laponie) {Laponie};

\begin{scope}[
        on background layer,
        every node/.style={fill=black!20,inner sep=1pt}, 
        ]
\node [circle,fit = (root) (aimeraient)] {};
\node [circle,fit = (root-3-3-2) (Laponie)] {};
\node [circle,fit = (root-1-2-2) (gens)] {};
\node [circle,fit = (root-3-2) (Noel)] {};

\fill [black!20] ($ (root-1) !.5! (root-1-1) $) ellipse [x radius=5mm, y radius=20mm, rotate=-33]; 
\fill [black!20] ($ (root-1-2) !.5! (root-1-2-1) $) ellipse [x radius=5mm, y radius=20mm, rotate=-33]; 
\fill [black!20] ($ (root-3-1) !.5! (root-3) $) ellipse [x radius=5mm, y radius=20mm, rotate=-55]; 
\fill [black!20] ($ (root-3-3-1) !.5! (root-3-3) $) ellipse [x radius=5mm, y radius=20mm, rotate=-33]; 
\end{scope}
\end{tikzpicture}}%
\smallskip\\
{\Large⇓}%
...
[aimeraient,name=aimeraient
  [beaucoup,name=beaucoup,edge label={node[near end,above] {2}}
    [de,name=de,edge label={node[midway,left] {1}}
        [gens,name=gens,edge label={node[midway,left] {1}}]
    ]
  ]
  [passer,name=passer,edge label={node[midway,above] {1}}
...
\node [fit=(beaucoup) (gensde), inner sep=2pt] (gensde) {};
\node [fit=(Laponie) (en), inner sep=0pt] (enLaponie) {};
\node [fit=(Noel) (passer), inner sep=0pt] (Noelpasser) {};
\node [fit=(Noelpasser) (enLaponie), inner sep=2pt] (passerLaponie) {};
\node [fit=(aimeraient) (passerLaponie), inner sep=3pt] (aimeraientLaponie) {};
\node [fit=(gensde) (aimeraientLaponie), inner sep=3pt] {};
\end{scope}
\end{forest}
...
\chevalier{Historique des représentations syntaxiques par des diagrammes en constituants}{%\label{sec:3.4.17}
    ...
    \todo[inline]{The structure of this graph was not contained in the original submission file}
...
\loupe{Binarité, construction et connexion}{%\label{sec:3.4.18}
    ...
    \begin{minipage}[c]{.3\linewidth}\centering
      \begin{tikzpicture} 
        \node [AbsSet,label=below:N] at (0,0) (A) {};
        \node [AbsSet,label=below:V] at (2,0) (B) {};
    ...
    \begin{tikzpicture}
      \node at (0,0) [AbsSet,label=below:N] (A) {};
      \node at (2,0) [AbsSet,label=below:V] (B) {};
      \node at (4,0) [AbsSet,label=below:N] (C) {};
