...
Une autre méthode consiste à construire d’abord un arbre de constituants. Cette méthode suppose que l’on ait repéré l’unité maximale que l’on veut analyser (ce qui n'est pas sans difficulté comme nous le verrons dans le \chapfuturef{21}). Appelons cette unité une phrase selon l’usage. Voici la méthode.
...
\begin{figure}[H]
  ...
  \todo[inline]{Reduce the edges in diagonal in subfig b and c. They must have the same length as the vertical ones, which could also be slightmly reduced. It would be nicer to put b and c next to each other and a above. In subfig b, the labels "agent" and "patient" must be higher to look on the middle of the edge.}
\end{figure}
...
\corrections{%\label{sec:3.4.32}
    ...
    \todo[inline]{Can you use the usual font for the words? Yes, please!}
    \end{figure}
