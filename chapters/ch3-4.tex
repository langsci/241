...
\loupe{Binarité, construction et connexion}{%\label{sec:3.4.18}
   ...
   \label{fig:subjectale} Trois représentations de la construction subjectale\smallskip\\
...
\ea
\ea   Pierre mange le pain.
\ex   Pierre mange la nuit. 
\z
\z
...
\loupe{Le groupe verbal}{%\label{sec:3.4.20}
    ...
    \ea
    Configurations syntaxiques caractérisant respectivement VP1 et VP2\smallskip\\
...
\begin{figure}
\caption{\label{fig:}Arbre de constituants binaire (avec têtes)\todo[inline]{Is this the same figure as above? If yes, maybe print only once?}}
\begin{tikzpicture}
\begin{scope}[
        every node/.style={CircleFillNode},
        level distance=2\baselineskip,
...
Arbre de constituants avec marquage partiel des têtes\medskip\\
\begin{tikzpicture}[every node/.style={font=\strut}]
    \begin{scope}[
    every node/.style={CircleFillNode},
    sibling distance=15mm,
    ...
    Structure de dépendance associée\medskip\\
    \begin{forest} for tree={font=\itshape}
    [dort
      [,nice empty nodes
        [le] [chien [petit]]
...
\loupe{Deux types d’arbres de constituants}{%\label{sec:3.4.26}
    ...
    \ea Branchement ternaire et correspondance par la méthode de Lecerf\\
    ...
    Dans un arbre binaire, il en va tout autrement, puisque, comme nous l’avons montré ci-dessus (\sectref{sec:3.4.21} sur \textit{Arbres de constituants binaire et polygraphe}), chaque branchement correspond à une unique connexion. Si un tel arbre contient un branchement ternaire, il doit être interprété comme une \textbf{connexion ternaire} (voir l’\encadref{fig:3.2.17} \textit{Connexion binaire} ?). La syntaxe X-barre, qui revendique l’usage des arbres binaires, s’est aventurée à proposer des branchements ternaires pour la coordination (voir \citealt{jackendoff1977x}, mais on trouve déjà les mêmes arbres dans \citealt{hockett1958course} ; voir ci-dessous les boîtes de Hockett pour la coordination où le branchement est ternaire et symétrique, avec une position bien différenciée pour la conjonction).
...
Nous allons prendre un exemple (on peut démarrer avec des exemples plus simples~\HappySmiley) (tiré des \textit{Fleurs du mal} de Beaudelaire, \textit{La vie antérieure}) :
...
\loupe{Constituance ou dépendance ?}{%{\label{sec:3.4.29}
    ...
    \subsection{Constructions exocentriques    }
    ...
    \subsection{Interface sémantique-syntaxe}
    ...
    \ea Arbres de dépendance avec portée\\
    \ea \textit{les voitures coréennes chères}\\
    \begin{forest} for tree = {font=\itshape}
      [voitures
    ...
    \end{forest}
    \ex \textit{les voitures chères coréennes}\\
    \begin{forest} for tree = {font=\itshape}
      [voitures
