...
\begin{figure}
\begin{tikzpicture}
\matrix (matrix) [matrix of nodes,row sep={1.5cm,between origins}] {
    `Ali' & & `durer' & & `deux'\\
    & `malade' & & `semaine'\\
};
\draw[-{Triangle[]}] (matrix-1-5) -- (matrix-2-4) node[font=\footnotesize,midway,left] {1};
\draw[-{Triangle[]}] (matrix-1-3) -- (matrix-2-4) node[font=\footnotesize,midway,right] {2};
\draw[-{Triangle[]}] (matrix-1-3) -- (matrix-2-2) node[font=\footnotesize,midway,left] {1};
\draw[-{Triangle[]}] (matrix-2-2) -- (matrix-1-1) node[font=\footnotesize,midway,left] {1};
\end{tikzpicture}
    ...
    \begin{scope}[every node/.style={circle,draw},level distance=2\baselineskip]
      \node (root) {}
        child { node{} 
            ...
            edge from parent[->] node[left,reset shape] {1} }
        child { node{} 
            child { node{} edge from parent[<-] node[right,reset shape] {1} } 
...
\loupe{La Théorie Sens-Texte}{%\label{sec:1.3.9}
    ...
    % \begin{figure}
    \ea \begin{forest}for tree={edge={double,{Latex[open,length=5pt]}-{Latex[open,length=5pt]}}}
