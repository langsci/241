...
À chaque fois, on obtient une \textstyleTermes{bribe}, c’est-à-dire un segment inachevé qui est ensuite corrigé par le locuteur. La disposition suivante du texte, dite \textstyleTermes{analyse en grille}, permet de mettre en évidence l’\textstyleTermes{entassement} (voir le \chapfuturef{sec:1.1.9}) dans la même position syntaxique de la bribe et du segment qui vient la remplacer :
...
Plus étonnant est le phénomène de la \textstyleTermes{greffe} bien étudié par José Deulofeu (voir le \chapfuturef{21}). L’exemple suivant en fourni deux :
...
\ea donc pour essayer un petit peu de sortir cette personne de la misère (car c’est vraiment un petit peu semblable aux Misérables de Victor-Hugo) 
...
\end{tabular}\todo[inline]{1) "nous essayons tant bien que mal de lui faire comprendre que \textbf{sa cabane}" must be in the table, aligned with "on essaye de lui faire comprendre qu’elle ne pourra plus vivre dans cette cabane" ; 2) the two lines after "quand elle aura" are independent from the two lines after  "donc le jour"; in other words the vertical line must be in two parts }
\z
