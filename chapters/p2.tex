...
\maths{Dualité}{%\label{sec:2.2.8}
    ...
    \begin{figure}
    \caption{\color{red}Please add a caption}
...
\end{tikzpicture}
\caption{Découpage de /\textstylePhono{avãs-}/ selon les axes paradigmatique et syntagmatique\todo[inline]{Would you like to have vertical rules left and right of ``avance'' in every line?}}
\end{figure}
...
\begin{styleLivreImportant}
Un sémantème qui est la combinaison de plusieurs syntaxèmes est appelé un \textstyleTermes{phrasème}. Les phrasèmes sont encore appelés des \textstyleTermes{locutions} ou des \textstyleTermes{expressions figées} dans la tradition grammaticale française et des \textstyleTermes{expressions idiomatiques} (angl. \textit{idioms}) dans la tradition anglo-saxonne. Les délimiteurs $⌜$…\textsuperscript{ ⌉} servent à indiquer qu’une combinaison de syntaxèmes forme un phrasème.
\end{styleLivreImportant}
...
\begin{tikzpicture}
 \graph [grow down, branch right sep=2cm,edges=black!20,nodes=black!20] 
   {
     {‘se déplacer’ [black], ‘se sentir’, ‘futur’  …}
     -- {ALLER [black,xshift=4cm]}
...
\begin{tikzpicture}
 \graph [grow down, branch right sep=2cm,edges=black!20,nodes=black!20] 
   {
     {‘se déplacer’ [black], ‘se sentir’ [black], ‘futur’ … [black]}
     -- {ALLER [black,xshift=4cm]}
...
\begin{tikzpicture}
 \graph [grow down, branch right sep=2cm,edges=black!20,nodes=black!20] 
   {
     {‘se déplacer’, ‘se sentir’ [black], ‘futur’ …}
     -- {ALLER [black,xshift=4cm]}
...
\begin{figure}
\begin{forest} calign=child, calign primary child=2, calign secondary child=4
  [ALLER,s sep=2cm,name=ALLER
    [\textit{/al/-},name=al] [\textit{/v/-},name=v]  [\textit{/i/-}] [\textit{\strut/aj/-},name=aj]
  ]
  \node [fit=(ALLER) (v), draw, inner sep=0pt] {};
  \node [fit=(ALLER) (al) (aj), draw, inner sep=2pt] {};
\end{forest}
\caption{morphe et morphème\label{fig:}}
\end{figure}

\begin{figure}
\begin{forest} calign=child, calign primary child=2
  [ALLER, grow=north, s sep=2cm,name=ALLER
    [‘se déplacer’]  [‘se sentir’,name=sentir] [‘futur’  …]
  ]
\node [fit=(ALLER) (sentir), draw, inner sep=0pt] {};
\end{forest}
\caption{sémantème}    
\end{figure}
